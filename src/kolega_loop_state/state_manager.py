...
class AttemptLimitExceeded(Exception):
    ...
    def __init__(self, attempts_made: int, max_attempts: int):
        ...
        super().__init__(
            f"Attempt limit exceeded: {attempts_made}/{max_attempts}"
        )
...
class WorkLog:
    ...
    def load(cls, path: str = "work-log.json") -> "WorkLog":
        ...
        if p.exists():
            try:
                ...
                import sys
                print(
                    f"[loop-state] WARNING: work-log.json corrupted ({exc}). "
                    f"Reinitializing.",
                    file=sys.stderr,
                )
    ...
    def _validate(self) -> None:
        """Basic schema validation. Raises ValueError on bad data."""
        if not isinstance(self._data, dict):
            raise ValueError("work-log.json must be a JSON object")
        if self._data.get("version") != "1.0":
            raise ValueError(
                f"Unsupported work-log version: {self._data.get('version')}"
            )
    ...
    def save(self) -> None:
        ...
        fd, tmp = tempfile.mkstemp(
            dir=str(p.parent), prefix=".work-log-", suffix=".tmp"
        )
        try:
    ...
    def inc_attempt(self) -> int:
        """Increment attempts_made. Raises AttemptLimitExceeded if over limit."""
        self._data["attempts_made"] += 1
        self.save()

        if self._data["attempts_made"] > self._data["max_attempts"]:
            raise AttemptLimitExceeded(
                self._data["attempts_made"], self._data["max_attempts"]
            )
    ...
    def revert(self) -> str:
        ...
        if backup and os.path.isdir(backup):
            cwd = os.getcwd()
            return (
                f"rsync -a --delete {shlex_quote(backup)}/ {shlex_quote(cwd)}/"
            )
    ...
    def backup_current(self) -> str:
        ...
        if self._is_git_repo():
            # Git mode: just record HEAD — revert will use it
            commit = self._git_head()
            self._data["last_green_commit"] = commit
            self.save()
            return commit

        # No-Git mode: copy working tree
        ...
        # Copy everything except the backup dir itself and work-log.json
        ignore = shutil.ignore_patterns(
            ".loop-backup-*", "work-log.json", ".git", "node_modules",
    ...
    def get_anti_patterns(self, for_module: Optional[str] = None) -> list:
        ...
        if for_module:
            aps = [
                ap
                for ap in aps
                if for_module.lower() in ap.get("file", "").lower()
            ]
        return aps
...
# ------------------------------------------------------------------
# shlex_quote fallback (stdlib in Python 3.3+)
# ------------------------------------------------------------------
try:
    from shlex import quote as shlex_quote
except ImportError:
    def shlex_quote(s: str) -> str:
        """Minimal shell quoting for paths without spaces."""
        if " " in s:
            return f"'{s}'"
        return s
